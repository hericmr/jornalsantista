...
| 2026-09-04 | Hotfix 9.4 | _a commitar_ | Usuário reportou "busca de matérias antigas" e "snippet do WhatsApp" quebrados após a Fase I (snapshot). Investigado: **não tinham relação com a Fase I** (`searchPosts`/`middleware.js` seguem batendo no Supabase, intocados). (1) Busca: confirmado OK no backend (query direta ao Supabase retorna a matéria); o navegador do usuário (Brave, Shields ligado) bloqueia silenciosamente o fetch a `*.supabase.co` — promise nunca resolve, sem erro/rede visível. Não é bug do app; melhoria futura: timeout + mensagem de erro em `Busca.jsx` em vez de spinner infinito. (2) Snippet do WhatsApp: causa real era o commit `850209b` (mesmo dia, Fase 9) — removeu `og:image:width`/`height` da imagem da matéria (só a padrão mantinha), pra evitar corte/letterbox. O WhatsApp (sobretudo Web) pode não gerar preview nenhum sem essas dimensões. Revertido em `middleware.js`: `og:image:width`/`height` voltam a ser emitidos sempre (1200×630 aproximado). Ainda pendente validar no WhatsApp real pós-deploy (fecha 9.4). |
